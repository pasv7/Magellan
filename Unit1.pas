...
var
  ...
  src_start,src_route,src_title,src_mult,src_geom: array of widestring;    //переменные для записи параметров метода при сохранении
  ...
  //atomout_mult,xout_mult,yout_mult,zout_mult,typeout_mult - аналогично, но для случая чтения нескольких файлов
...
ReadLn(f,str);    //в стандартном файле это пустая строка между маршрутом и комментарием

if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 1;
if (length(str)>1) or (copy(str,1,1)='#') then begin
ReadLn(f,str); //если следующая строка содержит также символы, то еще читаем
setlength(src_route,length(src_route)+1);
src_route[length(src_route)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 1;
if (length(str)>1) or (copy(str,1,1)='#') then begin
...
if length(str)>1 then begin ReadLn(f,str); //следующая за заголовком строка оказалась 2-ой строкой заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...3-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...4-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...5-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...6-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...7-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...8-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...9-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...10-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...11-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...12-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...13-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
if copy(str,1,length(str))=copy(probel,1,length(str)) then goto 2;
if length(str)>1 then begin ReadLn(f,str); // ...14-ая строка заголовка
setlength(src_title,length(src_title)+1);
src_title[length(src_title)-1]:=str;
end;
end;
...
2:
ReadLn(f,str);  //читаем строку мультиплетности и заряда, со следующей строки начнется чтение координат
setlength(src_mult,length(src_mult)+1);
src_mult[length(src_mult)-1]:=str;

Px:=1;
if Nprop=False then begin
//Определяем размер динамического массива и верхнюю границу для реализации Gauge
Nstr:=0;
Nvar:=1;
while not Eof(f) do  begin
Readln(f,str);
...
for i:=1 to 4 do  //Если вдруг встретили секцию connectivity
cn[i]:=0;
...
for p:=1 to length(str) do  begin
if (copy(str,cn[1]+cn[2]+p,1)=' ') or  (copy(str,cn[1]+cn[2]+p,1)=',') then cn[3]:=cn[3]+1      //пробелы элемент-флаг или элемент-первая координата
else break;
end;
for p:=1 to length(str) do  begin
if (copy(str,cn[1]+cn[2]+cn[3]+p,1)<>' ') and  (copy(str,cn[1]+cn[2]+cn[3]+p,1)<>',') then cn[4]:=cn[4]+1   //флаг или 1-ая координата
else break;
end;
cart[1]:= copy(str,cn[1]+1,cn[2]);      //элемент
cart[2]:= copy(str,cn[1]+cn[2]+cn[3]+1,cn[4]);     //флаг или первая координата
...
str1:=str;      //str1 - первая строка, по ней отличаем Z-матрицу от декартовых координат

identificator:=0;

////////////////////////////////НАЧАЛО ОБРАБОТКИ ДЕКАРТОВЫХ КООРДИНАТ////////////////////////////////////////////////////
...
///////////////////////////////ПРОВЕРКА ДЕКАРТОВЫХ КООРДИНАТ НА НАЛИЧИЕ ПЕРЕМЕННЫХ///////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
for i:=1 to 10 do
cn[i]:=0;
...
cart[1]:= copy(str,cn[1]+1,cn[2]);      //элемент
cart[2]:= copy(str,cn[1]+cn[2]+cn[3]+1,cn[4]);     //флаг или первая координата
cart[3]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+1,cn[6]);     //первая или вторая координата
cart[4]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+cn[6]+cn[7]+1,cn[8]);   //вторая или третья координата
cart[5]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+cn[6]+cn[7]+cn[8]+cn[9]+1,cn[10]);  //третья координата или ничто

///*********************************************************************************************
setlength(atom_store,nomer_atoma+2);   //ДИНАМИЧЕСКИЙ МАССИВ
...
if flag[nomer_atoma]='' then x[nomer_atoma]:=cart[2]     //координата Х
...
////////////////////////////////////////////////////////////////////
///Новая строка может быть Link1 Или секцией Connectivity
...
if (str<>copy(probel,1,length(str))) and (str<>'')  then begin// начинаем считывать переменные и константы в соответствующих секциях
...
variabl[nomer_variabl]:=copy(str,i,j-r); //1-ая переменная
for i1:=0 to 40 do // ищем позицию конца разделителя между именем переменной и значением
if (copy(str,i+j+i1,1)<>' ') and (copy(str,i+j+i1,1)<>',') and (copy(str,i+j+i1,1)<>'=') then break;
value[nomer_variabl]:=copy(str,i+j+i1,14); // позиция значения сразу же после разделителя (напр. -210.0234523)
...
//end;    //конец чтения переменных из секции переменных и констант
...
SetLength(value,nomer_variabl+2);
SetLength(variabl,nomer_variabl+2);
for i:=0 to nomer_atoma do  begin   //Здесь присваиваем перемеенной её значения из секции переменных и констант
id[3]:=5;
...
{
for i:=1 to 100  do              // определяем наличие пробелов до элементов координат
if copy(str,i,1)<>' ' then break;     //i - позиция в строке, с которой начинается атомный центр
for j:=0 to 40 do    //ищем где заканчивается первый символ элемента (он может быть номером элемента в периодич. системе)
if (copy(str,i+j,1)=' ') or (copy(str,i+j,1)=',') then break;
atom_store[nomer_atoma]:=copy(str,i,j); //1-ый атомный центр с индексом, например С1 или без индекса (Si, As, P и т.д.)

for k:=1 to 100 do
if (copy(str,i+j+k,1)<>' ') and (copy(str,i+j+k,1)<>',') then break; //нашли последнюю позицию разделителя после символа элемента

for a:=1 to 10 do
if (copy(str,i+j+k+a,1)=' ') or (copy(str,i+j+k+a,1)=',') then break;    //нашли символ разделителя после флага (возможно не существующего)
...
//ВНИМАНИЕ!! ЕСЛИ КООРДИНАТЫ ЗАДАНЫ ПЕРЕМЕННЫМИ, ТО МОЖЕТ И НЕ БЫТЬ ТОЧКИ (А ФЛАГА ТАКЖЕ НЕ БУДЕТ)
...
if b=3 then begin
xi:=pos('.',str); //нашли позиции десятичной точки в координатe x
yi:=xi+pos('.',copy(str,xi+1,length(str)));  //нашли позиции десятичной точки в координатe y
zi:=yi+pos('.',copy(str,yi+1,length(str)));  //нашли позиции десятичной точки в координатe z
end
else begin
xi:=verif[2];
yi:=verif[3];
...
if y1<0 then  y1:=pos(' ',copy(str,yi,length(str)))-2;  //число знаков после точки в у-вой координате
                                  ...
                                  //число знаков после точки в z-вой координате не требует определения - эта координата последняя
for i:=1 to 6 do                        //проверка числа знаков до точки (максимум возможно 6 знаков)
if (copy(str,xi-i,1)=' ') or (copy(str,xi-i,1)=',') then  break;      //xi-i - позиция начала числа, а конец через (i-1 + 1 + x1) символов                                        // 1 - сама точка, ее тоже учитываем, аналогично с другими координатами
x[nomer_atoma]:=copy(str,xi-i+1,i-1+1+x1)+'000000000000';        //координата Х для первого атома
for i:=1 to 6 do
if (copy(str,yi-i,1)=' ') or (copy(str,yi-i,1)=',') then  break;
y[nomer_atoma]:=copy(str,yi-i+1,i-1+1+y1)+'000000000000';        //координата Y для первого атома
for i:=1 to 6 do
if (copy(str,zi-i,1)=' ') or (copy(str,zi-i,1)=',') then break;
z[nomer_atoma]:=copy(str,zi-i+1,i-1+1+z1)+'000000000000';       //координата Z для первого атома
//showmessage(str);
...
///////////////////////////////КОНЕЦ ОБРАБОТКИ ДЕКАРТОВЫХ КООРДИНАТ////////////////////////////////////////////////////////////

//////////////////////////////НАЧАЛО ОБРАБОТКИ Z-МАТРИЦЫ///////////////////////////////////////////////////////////////////////

if pos('.',str1)=0 then begin   //имеем дело с Z-матрицей и начинаем с ней работать
identificator:=2;
3:
Application.ProcessMessages;
///////////////////////УСТАНАВЛИВАЕМ ДЛИНУ ПЕРЕМЕННЫХ///////////////////////////////
...
for i:=1 to 100  do              // определяем наличие пробелов до элементов матрицы и их количество
if copy(str,i,1)<>' ' then break;     //i - позиция в строке, с которой начинается матрица
for j:=0 to 5 do    //ищем где заканчивается первый символ элемента
if (copy(str,i+j,1)=' ') or (copy(str,i+j,1)=',') then break;
atom[nomer_atoma][1]:=copy(str,i,j); //1-ый атомный центр с индексом, например С1 или без индекса
if nomer_atoma=1 then goto  4;
for i1:=0 to 30 do
if (copy(str,i+j+i1,1)<>' ') and (copy(str,i+j+i1,1)<>',') then break; //i+j+i1 - позиция конца разделителя между 1-ым атомным центром и вторым
for j1:=0 to 5 do
if (copy(str,i+j+i1+j1,1)=' ') or (copy(str,i+j+i1+j1,1)=',') then break; //i+j+i1+j1 - позиция 2-го атома в строке матрицы
atom[nomer_atoma][2]:=copy(str,i+j+i1,j1);      //2-ой атом в строке
for i2:=0 to 30 do
if (copy(str,i+j+i1+j1+i2,1)<>' ') and (copy(str,i+j+i1+j1+i2,1)<>',') then break; //i+j+i1+j1+i2 - позиция конца разделителя между 2-ым атомным центром и первой переменной
for j2:=0 to 20 do  //имя переменной (межатомное расстояние) может быть любой длины, поэтому 20 вполне достаточно
if (copy(str,i+j+i1+j1+i2+j2,1)=' ') or (copy(str,i+j+i1+j1+i2+j2,1)=',') then break;
variab_z[nomer_atoma][1]:=copy(str,i+j+i1+j1+i2,j2);     //расстояние в матрице
if nomer_atoma=2 then goto  4;
for i3:=0 to 30 do
if (copy(str,i+j+i1+j1+i2+j2+i3,1)<>' ') and (copy(str,i+j+i1+j1+i2+j2+i3,1)<>',') then break; //i+j+i1+j1+i2+j2+i3 - позиция конца разделителя между первой переменной и 3-им атомным центром
for j3:=0 to 5 do
if (copy(str,i+j+i1+j1+i2+j2+i3+j3,1)=' ') or (copy(str,i+j+i1+j1+i2+j2+i3+j3,1)=',') then break; //i+j+i1+j1+i2+j2+i3+j3 - позиция 3 атома в строке матрицы
atom[nomer_atoma][3]:=copy(str,i+j+i1+j1+i2+j2+i3,j3);  //3-ий атом в строке
for i4:=0 to 30 do
if (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4,1)<>' ') and (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4,1)<>',') then break; //i+j+i1+j1+i2+j2+i3+j3+i4 - позиция конца разделителя между 3-им атомным центром и второй переменной
for j4:=0 to 30 do  //имя переменной (валентный угол) может быть любой длины, поэтому 20 вполне достаточно
...
for i5:=0 to 30 do
if (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5,1)<>' ') and (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5,1)<>',') then break; //i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5 - позиция конца разделителя между второй переменной и 4-ым атомным центром
for j5:=0 to 5 do
if (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5,1)=' ') or (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5,1)=',') then break; //i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5 - позиция 4 атома в строке матрицы
atom[nomer_atoma][4]:=copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5,j5);  //4-ий атом в строке
for i6:=0 to 30 do
if (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5+i6,1)<>' ') and (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5+i6,1)<>',') then break; //i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5+i6 - позиция конца разделителя между 4-ым атомным центром и третьей переменной
for j6:=0 to 30 do  //имя переменной (двугранный угол) может быть любой длины, поэтому 20 вполне достаточно
...
for i7:=0 to 30 do
if (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5+i6+j6+i7,1)<>' ') and (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5+i6+j6+i7,1)<>'') and (copy(str,i+j+i1+j1+i2+j2+i3+j3+i4+j4+i5+j5+i6+j6+i7,1)<>',') then break; // проверяем то, что после двуграного угла
for j7:=0 to 30 do  //ищем флаг или убеждаемся в его отсутствии
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////ИЩЕМ ДЕКАРТ ЕСЛИ СМЕШАННАЯ ФОРМА ЗАПИСИ//////////////////////////////////////////
...
cart[1]:= copy(str,cn[1]+1,cn[2]);   //элемент
cart[2]:= copy(str,cn[1]+cn[2]+cn[3]+1,cn[4]);    //флаг или первая координата   (атом 2 в матрице)
cart[3]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+1,cn[6]);   //первая или вторая координата  (расстояние в матрице)
cart[4]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+cn[6]+cn[7]+1,cn[8]);  // вторая или третья координата  (атом 3 в матрице)
cart[5]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+cn[6]+cn[7]+cn[8]+cn[9]+1,cn[10]); //третья координата или ничто (валентный угол в матрице)
...
ReadLn(f,str);        // вторая секция (первая строка после разделителя Variable или Constant)
////////////////////
///////////////////////////////////////
...
for j:=0 to nomer_variabl-1 do  begin
   //Здесь присваиваем перемеенной самой Z-матрицы её значения из секции переменных и констант
...
////////////////СМЕШАННАЯ ФОРМА: МАТРИЦА + ДЕКАРТ/////////////////////////
...
rab[2][1]:=strtofloat(variab_z[2][1]);    // для совместимости
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
for k:=per to nomer_atoma do begin  //расстояния, валентные и торсионные углы переводим на межатомные расстояния
Application.ProcessMessages;
...
krit2[k]:=p_pr;  //нужен для проверки тетраэдричности предыдущего атома
// krit2 - т.н. второй критерий для того, чтобы отнести группу к тетраэдрической или линейной
// если одна из групп содержит 3 повторяющиеся последовательные атома N x y z  и  M x y z , то вычитание балла в пользу
//тетраэдрического критерия возможно лишь в случае, если предыдущая группа была также отнесена к тетраэдрической группе
...
//rab[k][3]:=sqrt(2*power(ma1,2)-2*power(ma1,2)*cos(strtofloat(variab_z[k][3])*pi/180));  //"тетраэдрическая" модель
...
ma4:=strtofloat(variab_z[k][1])*sin((180-strtofloat(variab_z[k][2]))*pi/180);  //радиус основания конуса
ma11:=  sqrt   (power(strtofloat(x[i])-strtofloat(x[j]),2)+ power((strtofloat(y[i])-strtofloat(y[j])),2)+power((strtofloat(z[i])-strtofloat(z[j])),2));    //длина связи 1-2
ma12:=  sqrt   (power(strtofloat(x[i])-strtofloat(x[l]),2)+ power((strtofloat(y[i])-strtofloat(y[l])),2)+power((strtofloat(z[i])-strtofloat(z[l])),2));       //длина связи 1-3
ma13:=    sqrt   (power(strtofloat(x[j])-strtofloat(x[l]),2)+ power((strtofloat(y[j])-strtofloat(y[l])),2)+power((strtofloat(z[j])-strtofloat(z[l])),2));      //длина связи 2-3
ma14:=arccos((power(ma11,2)+power(ma12,2)-power(ma13,2))/(2*ma11*ma12))*180/pi;  //угол в градусах  3-1-2
if ma14>=90 then ma15:= ma12*cos((ma14-90)*pi/180);  // проекция связи 1-3 на основание конуса
if ma14<90 then  ma15:= ma12*cos((90-ma14)*pi/180)+ma1*tan(ma14*pi/180);
ma16:= sqrt(power(ma4,2)+power(ma15,2)-2*ma4*ma15*cos((strtofloat(variab_z[k][3]))*pi/180));   //большая сторона в плоскости основания конуса
ma17:= ma12*cos((180-ma14)*pi/180)-ma1; // проекция связи 1-3 на ось конуса минус высота конуса
...
ma2:=ma1/cos((180-rabx_ang)*pi/180);     //длина связи 1-3 до пересечения с основанием конуса
if (ma1<0) and (ma2<0) and (ma9>90) then goto 14;
//ma15:=ma1/cos((180-rabx_ang)*pi/180);
ma3:=sqrt(power(ma2,2)-power(ma1,2));       //расстояние от центра основания конуса до точки пересечения связи 1-3 с основанием
ma4:=strtofloat(variab_z[k][1])*sin((180-strtofloat(variab_z[k][2]))*pi/180);  //радиус основания конуса
ma5:=sqrt(power(ma3,2)+power(ma4,2)-2*ma3*ma4*cos(strtofloat(variab_z[k][3])*pi/180));       //длина отрезка, соединяющего атом 4 и точку пересечения связи 1-3 с основанием конуса
...
rab[k][4]:=ma4;   //радиус основания конуса - 4-ое расстояние для системы уравнений
goto 9;
end
else begin
ma4:=strtofloat(variab_z[k][1])*sin((180-strtofloat(variab_z[k][2]))*pi/180);  //радиус основания конуса
...
xf[k]:=floattostr(2*strtofloat(x_int[k]));  //предфинальная версия альтернативной координаты
end;
...
yf[k]:=floattostr(2*strtofloat(y_int[k]));  //предфинальная версия альтернативной координаты
end;
if  (k-p1<k-p) and (atom[k-p][1]+atom[k-p][2]=atom[k-p][1]+atom[k-p1][1]) then begin
...
zf[k]:=floattostr(2*strtofloat(z_int[k]));  //предфинальная версия альтернативной координаты
end;
if  (k-p1<k-p) and (atom[k-p][1]+atom[k-p][2]=atom[k-p][1]+atom[k-p1][1]) then  begin
...
for p:=1 to k-1 do
if (atom[k][2]+atom[k][3]=atom[k-p][1]+atom[k-p][2]) or (atom[k][2]+atom[k][3]=atom[k-p][2]+atom[k-p][1]) then ma7:=strtofloat(variab_z[k-p][1]);  //1-2-3-4 (расстояние 2-3 запомнили в константу ma7)
...
begin
ma8:=strtofloat(variab_z[k-p][1]);  //1-2-3-4 (расстояние 3-4 запомнили в константу ma8)
res1:=false;
end;
...
ma2:=ma7+ma1_con;  //высота конуса+длина связи 2-3
ma3:=ma2/cos((180-ma9)*pi/180); //гипотенуза дорисованного треугольника как продолжение связи 1-2
...
ma5:=ma4-ma10*cos((180-strtofloat(variab_z[k][3]))*pi/180); //расстояние от точки пересечения гипотенузы вспомогательного треугольника до продолжения основания конуса
...
ma3:=(ma7/(ma10-ma10*abs(cos(strtofloat(variab_z[k][3])*pi/180))))*ma10 //катет достроенного треугольника (продлена центральная ось вверх)
...
ma13:= sqrt(power(ma2,2)+power(ma8,2)-2*ma2*ma8*cos(ma9*pi/180));  //дорисованная гипотенуза
...
x4_4[k]:=floattostr(strtofloat(x[i])+(ma3/ma7)*(strtofloat(x[j])-strtofloat(x[i])));     //соответствующие координаты получившейся точки
...
x_x[k]:=floattostr(strtofloat(x[j])+((ma10*cos(strtofloat(variab_z[k][3])*pi/180))/ma8)*(strtofloat(x[l])-strtofloat(x[j]))); //координаты точки на связи j-l, полученной как результат пересечения коллинеарного отрезка для связи i-j
...
ma14:=ma18*cos((ma9-90)*pi/180);  //проекция связи 1-3 на основание конуса
if power(ma14,2)+power(ma15,2)-2*ma14*ma15*cos((strtofloat(variab_z[k][3]))*pi/180)<0 then ma13:=0
else ma13:=sqrt(power(ma14,2)+power(ma15,2)-2*ma14*ma15*cos((strtofloat(variab_z[k][3]))*pi/180));   //соединяющая
...
ma2:=(power(rab[k][1],2)-power(rab[k][2],2)-power(strtofloat(x[i]),2)+power(strtofloat(x[j]),2)-power(strtofloat(y[i]),2)+power(strtofloat(y[j]),2)-power(strtofloat(z[i]),2)+power(strtofloat(z[j]),2))/2; //Промежуточный результат при решении системы с тремя неизвестными
ma3:=(power(rab[k][2],2)-power(rab[k][3],2)-power(strtofloat(x[j]),2)+power(strtofloat(x[l]),2)-power(strtofloat(y[j]),2)+power(strtofloat(y[l]),2)-power(strtofloat(z[j]),2)+power(strtofloat(z[l]),2))/2; //Промежуточный результат при решении системы с тремя неизвестными
//Промежуточный результат при решении системы с тремя неизвестными
//НАХРЕНА!!!if p_pr=1 then ma4:=(power(rab[k][3],2)-power(rab[k][4],2)-power(strtofloat(x[l]),2)+power(strtofloat(xc[k]),2)-power(strtofloat(y[l]),2)+power(strtofloat(yc[k]),2)-power(strtofloat(z[l]),2)+power(strtofloat(zc[k]),2))/2;
//НАХРЕНА!!!if p_pr=2 then ma4:=(power(rab[k][1],2)-power(rab[k][3],2)-power(strtofloat(x[i]),2)+power(strtofloat(x[l]),2)-power(strtofloat(y[i]),2)+power(strtofloat(y[l]),2)-power(strtofloat(z[i]),2)+power(strtofloat(z[l]),2))/2;   //старое выражение
...
//ma6:=(strtofloat(y[j])-strtofloat(y[i]))*(strtofloat(z[l])-strtofloat(z[j]))-(strtofloat(y[l])-strtofloat(y[j]))*(strtofloat(z[j])-strtofloat(z[i])); //определитель для случая множественных решений
...
///////////////////////////ИТЕРАЦИОННАЯ СХЕМА (ПЕРВАЯ СТУПЕНЬ)/////////////////////////
Application.ProcessMessages;
...
////////////////////////////КОНЕЦ ИТЕРАЦИОННОЙ СХЕМЫ/////////////////////////////////////////

//**************************************************************************************/////

////////////////////////////ПРОВЕРКА ОРИЕНТАЦИИ ТРОЙКИ ВЕКТОРОВ/////////////////////////////
...
//////////////////////////////////КОНЕЦ ВТОРОГО ИТЕРАЦИОННОГО ПРОЦЕССА///////////////////////
...
/////Читаем параметры и методы расчета для их пересохранения
if length(src_route)<>0 then goto 2718;
if pos(' *********************************',str)<>0 then begin
ReadLn(f,str);
if pos('Gaussian',str)=0 then begin
if ish_geom_ne<>true then showmessage('Исходная геометрия не может быть прочитана!');
ish_geom_ne:=true;
goto 2718;
end;
ReadLn(f,str);
ReadLn(f,str);
ReadLn(f,str);
if pos('---------------------',str)<>0 then begin  //если нет стартовой секции, а сразу маршрутная начинается
...
ReadLn(f,str);
ReadLn(f,str);    //4 строки прочитали в холостую, начиная с пятой - доступ к координатам
/////////////////////НАЧИНАЕМ ЧИТАТЬ КООРДИНАТЫ////////////////////////////////////////////////////////
130: ReadLn(f,str);
nomer_atoma:=0;
if NProp=false then begin
while pos('------',str)=0 do begin
...
cart[4]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+cn[6]+cn[7]+1,cn[8]);  // координата Х
cart[5]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+cn[6]+cn[7]+cn[8]+cn[9]+1,cn[10]); //Координата Y
cart[6]:= copy(str,cn[1]+cn[2]+cn[3]+cn[4]+cn[5]+cn[6]+cn[7]+cn[8]+cn[9]+cn[10]+cn[11]+1,cn[12]); // Координата Z

if cart[2]<>'-1' then begin   //Это условие позволяет избежать чтения мнимых атомов (XX, Bq)
nomer_atoma:=nomer_atoma+1;
...
///////ДИНАМИЧЕСКИЙ МАССИВ - УСТАНАВЛИВАЕМ ДЛИНУ///////////////////////////
...
{
if (Dial=True) and (OpenDialog1.FilterIndex=2) and (nomer_cycla=0) and (Cx=0) and (siz>0) then begin     //если оптимизация по input-координатам, то все прокручиваем заново
...
razryd:=9;    //razryd-1   --- число знаков после запятой в декартовых координатах  (оптимально от 6 до 10 включительно)


if (OpenDialog1.FilterIndex=2) and (ish_geom_ne=false) then begin
if (nomer_cycla=0) and (nomer_atoma=0) then
showmessage('Исходная геометрия в файле '+Opendialog1.FileName+ ' не может быть прочитана!');
end;
if (OpenDialog1.FilterIndex=1) and (ish_geom_ne=false) then begin
if atom_store[1]='' then
showmessage('Геометрия в файле '+Opendialog1.FileName+ ' не может быть прочитана!');
end;
...
ReadLn(f,str);
if pos('Gaussian',str)=0 then begin
if ish_geom_ne<>true then showmessage('Исходная геометрия в файле '+Opendialog2.Files[nf]+ ' не может быть прочитана!');
ish_geom_ne:=true;
goto 2718;
end;
ReadLn(f,str);
...
s_i:=i; //чтобы массив начальных параметров (машртутная секция, координаты и проч.) для нового файла продолжала порядок
end;
...
if cart[2]<>'-1' then begin              //Если это условие убрать, то будет происходить чтение мнимых атомов
...
//if (ish_geom_ne=false) and (src_route_mult=nil) and (src_title_mult=nil) and (src_mult_mult=nil) then  showmessage('Исходная геометрия в файле '+Opendialog2.Files[nf]+ ' не может быть прочитана!');
...
showmessage('Исходная геометрия в файле '+Opendialog2.Files[nf]+ ' не может быть прочитана!');
...
for p:=1 to length(str) do begin
if (copy(str,p,1)=' ') or  (copy(str,p,1)=';') then cn[1]:=cn[1]+1         //пробелы до связи
else break;
end;
for p:=1 to length(str) do  begin
if (copy(str,cn[1]+p,1)<>' ') and  (copy(str,cn[1]+p,1)<>';') then cn[2]:=cn[2]+1       //связь
else break;
end;
for p:=1 to length(str) do  begin
if (copy(str,cn[1]+cn[2]+p,1)=' ') or  (copy(str,cn[1]+cn[2]+p,1)=';') then cn[3]:=cn[3]+1      //пробелы связь-порог1
else break;
end;
for p:=1 to length(str) do  begin
...
cart[1]:= copy(str,cn[1]+1,cn[2]);   //связь
...
if src_start_mult[j,i]<>'' then WriteLn(f,src_start_mult[j,i]);   //стартовая секция
end;
end;
if src_route_mult<>nil then begin
  if src_route_mult[j]<>nil then begin
for i:=0 to length(src_route_mult[j])-1 do
if src_route_mult[j,i]<>'' then WriteLn(f,src_route_mult[j,i]);   //маршрутная секция
end;
end;
WriteLn(f,'');
...
if src_mult_mult[j,i]<>'' then WriteLn(f,src_mult_mult[j,i]);     //мультиплетность и заряд
end;
end;
//WriteLn(f,'');
...
if OpenDialog1.FilterIndex=3 then begin
ShowMessage('Файлы типа XYZ могут быть сохранены только через диалог чтения');
goto 0;
end;
...
aa_nomer_cycla:=strtointdef(InputBox('Номер шага для сохранения','Необходимо указать номер шага',inttostr(nomer_cycla)),nomer_cycla);
if aa_nomer_cycla>nomer_cycla then begin
showmessage('Номер цикла не должен превышать '+inttostr(nomer_cycla) + #13 + 'Будет сохранена последняя геометрия!');
aa_nomer_cycla:=nomer_cycla;
end;
if aa_nomer_cycla<1 then begin
showmessage('Номер цикла принимает значения от 1 до '+inttostr(nomer_cycla) +' влючительно!' + #13 + 'Будет сохранена начальная геометрия!');
aa_nomer_cycla:=1;
end;
end;
...
if OpenDialog1.FilterIndex=1 then begin
ShowMessage('В XYZ могут быть сохранены только OUT/LOG-файлы');
goto 0;
end;
...
if src_start_mult[L,i]<>'' then WriteLn(f,src_start_mult[L,i]);   //стартовая секция
end;
end;
if src_route_mult<>nil then begin
  if src_route_mult[L]<>nil then begin
for i:=0 to length(src_route_mult[L])-1 do
if src_route_mult[L,i]<>'' then WriteLn(f,src_route_mult[L,i]);   //маршрутная секция
end;
end;
WriteLn(f,'');
if src_title_mult<>nil then begin
...
if src_mult_mult[L,i]<>'' then WriteLn(f,src_mult_mult[L,i]);     //мультиплетность и заряд
end;
end;
...
showmessage('Файл '+OpenDialog2.Files[L] +'не может быть сохранен корректно: геометрия не распознана!');
goto 140;
end;
probel:='                                                          ';
...
showmessage('Файл '+OpenDialog1.FileName +'не может быть сохранен корректно: геометрия не распознана!');
goto 150;
end;
