...
var
  ...
  ch,ch_at: word;       // ch - счетчик троек   , ch_at - счетчик атомов
...
///////////////////////////Обнуление всех переменных и массивов/////////////////////////////
pol1:='';
pol2:='';
pol3:='';
ch:=0;
...
if copy(str,p,1)=' ' then cn1:=cn1+1         //пробелы до элемента
else break;
end;
for p:=1 to length(str) do  begin
if copy(str,cn1+p,1)<>' ' then cn2:=cn2+1       //элемент
else break;
end;
for p:=1 to length(str) do  begin
if copy(str,cn1+cn2+p,1)=' '  then cn3:=cn3+1      //пробелы до 2-го элемента
else break;
end;
for p:=1 to length(str) do  begin
if copy(str,cn1+cn2+cn3+p,1)<>' '  then cn4:=cn4+1   //2-ой элемент
else break;
end;
for p:=1 to length(str) do  begin
if copy(str,cn1+cn2+cn3+cn4+p,1)=' ' then cn5:=cn5+1   //пробел до 3-го элемента
else break;
end;
for p:=1 to length(str) do  begin
if (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>' ') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>'') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>#13) then cn6:=cn6+1  //3-ий элемент
else break;
end;

pol1:= copy(str,cn1+1,cn2);   //первое значение
pol2:= copy(str,cn1+cn2+cn3+1,cn4);    //второе значение
pol3:= copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);   //третье значение

ReadLn(f,str);
11: ReadLn(f,str);
ReadLn(f,str);
ReadLn(f,str); // читаем до секции начала частот
...
ReadLn(f,str);  // прочитали строку   1    2     3 ...

p1:=0;          // а это для того, если предыдущая строчка не попала сразу на строку  1   2    3 ////*******
...
if  copy(str,1,15)<>'               ' then begin      /// контрольная
MessageDlg('Отсутствуют данные о колебаниях',mtWarning,[mbOK],0);
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
while str<>''  do begin      // читаем до конца секции частот   !!!!!!!!!!!!!!!!!!!!! начало начало начало
cn1:=0;
cn2:=0;
cn3:=0;
cn4:=0;
...
frn[ch]:= copy(str,cn1+1,cn2);   //первый номер
frn[ch+1]:= copy(str,cn1+cn2+cn3+1,cn4);    //второй номер
frn[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);   //третий номер
...
ReadLn(f,str);  // прочитали строку типов орбиталей  А     А    А  ...
setlength(orb,ch+3);
...
orb[ch]:= copy(str,cn1+1,cn2);   //первый тип МО
orb[ch+1]:= copy(str,cn1+cn2+cn3+1,cn4);    //второй тип МО
orb[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);   //третий тип МО
...
ReadLn(f,str);  // прочитали строку частот   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
setlength(freq,ch+3);
if pos('Frequencies --',str)<>0   then begin


for p:=length('Frequencies --')+2 to length(str) do begin
if copy(str,p,1)=' ' then cn1:=cn1+1         //пробелы до первой частоты
else break;
end;
for p:=length('Frequencies --')+2 to length(str) do  begin
if copy(str,cn1+p,1)<>' ' then cn2:=cn2+1       //первая частота
else break;
end;
for p:=length('Frequencies --')+2 to length(str) do  begin
if copy(str,cn1+cn2+p,1)=' '  then cn3:=cn3+1      //пробелы до второй частоты
else break;
end;
for p:=length('Frequencies --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+p,1)<>' '  then cn4:=cn4+1   //вторая частота
else break;
end;
for p:=length('Frequencies --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+cn4+p,1)=' ' then cn5:=cn5+1   //пробел до третьей частоты
else break;
end;
for p:=length('Frequencies --')+2 to length(str) do  begin
if (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>' ') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>'') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>#13) then cn6:=cn6+1  //третья частота
else break;
end;

freq[ch]:= copy(str,cn1+2+length('Frequencies --'),cn2);   //первая частота
freq[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Frequencies --'),cn4);    //вторая частота
freq[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Frequencies --'),cn6);   //третья частота
...
ReadLn(f,str);  // прочитали строку масс   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
setlength(mass,ch+3);
if pos('Red. masses --',str)<>0   then begin


for p:=length('Red. masses --')+2 to length(str) do begin
if copy(str,p,1)=' ' then cn1:=cn1+1         //пробелы до первой массы
else break;
end;
for p:=length('Red. masses --')+2 to length(str) do  begin
if copy(str,cn1+p,1)<>' ' then cn2:=cn2+1       //первая масса
else break;
end;
for p:=length('Red. masses --')+2 to length(str) do  begin
if copy(str,cn1+cn2+p,1)=' '  then cn3:=cn3+1      //пробелы до второй массы
else break;
end;
for p:=length('Red. masses --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+p,1)<>' '  then cn4:=cn4+1   //вторая масса
else break;
end;
for p:=length('Red. masses --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+cn4+p,1)=' ' then cn5:=cn5+1   //пробел до третьей массы
else break;
end;
for p:=length('Red. masses --')+2 to length(str) do  begin
if (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>' ') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>'') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>#13) then cn6:=cn6+1  //третья масса
end;

mass[ch]:= copy(str,cn1+2+length('Red. masses --'),cn2);   //первая масса
mass[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Red. masses --'),cn4);    //вторая масса
mass[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Red. masses --'),cn6);   //третья масса
...
ReadLn(f,str);  // прочитали строку констант   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
setlength(fr_const,ch+3);
if pos('Frc consts  --',str)<>0   then begin


for p:=length('Frc consts  --')+2 to length(str) do begin
if copy(str,p,1)=' ' then cn1:=cn1+1         //пробелы до первой константы
else break;
end;
for p:=length('Frc consts  --')+2 to length(str) do  begin
if copy(str,cn1+p,1)<>' ' then cn2:=cn2+1       //первая константа
else break;
end;
for p:=length('Frc consts  --')+2 to length(str) do  begin
if copy(str,cn1+cn2+p,1)=' '  then cn3:=cn3+1      //пробелы до второй константы
else break;
end;
for p:=length('Frc consts  --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+p,1)<>' '  then cn4:=cn4+1   //вторая константа
else break;
end;
for p:=length('Frc consts  --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+cn4+p,1)=' ' then cn5:=cn5+1   //пробел до третьей константы
else break;
end;
for p:=length('Frc consts  --')+2 to length(str) do  begin
if (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>' ') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>'') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>#13) then cn6:=cn6+1  //третья константа
end;

fr_const[ch]:= copy(str,cn1+2+length('Frc consts  --'),cn2);   //первая константа
fr_const[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Frc consts  --'),cn4);    //вторая константа
fr_const[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Frc consts  --'),cn6);   //третья константа
...
ReadLn(f,str);  // прочитали строку интенсивностей   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
setlength(ir_inten,ch+3);
if pos('IR Inten    --',str)<>0   then begin


for p:=length('IR Inten    --')+2 to length(str) do begin
if copy(str,p,1)=' ' then cn1:=cn1+1         //пробелы до первой интенсивности
else break;
end;
for p:=length('IR Inten    --')+2 to length(str) do  begin
if copy(str,cn1+p,1)<>' ' then cn2:=cn2+1       //первая интенсивность
else break;
end;
for p:=length('IR Inten    --')+2 to length(str) do  begin
if copy(str,cn1+cn2+p,1)=' '  then cn3:=cn3+1      //пробелы до второй интенсивности
else break;
end;
for p:=length('IR Inten    --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+p,1)<>' '  then cn4:=cn4+1   //вторая интенсивность
else break;
end;
for p:=length('IR Inten    --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+cn4+p,1)=' ' then cn5:=cn5+1   //пробел до третьей интенсивности
else break;
end;
for p:=length('IR Inten    --')+2 to length(str) do  begin
if (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>' ') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>'') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>#13) then cn6:=cn6+1  //третья интенсивность
end;

ir_inten[ch]:= copy(str,cn1+2+length('IR Inten    --'),cn2);   //первая интенсивность
ir_inten[ch+1]:= copy(str,cn1+cn2+cn3+2+length('IR Inten    --'),cn4);    //вторая интенсивность
ir_inten[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('IR Inten    --'),cn6);   //третья интенсивность
...
ReadLn(f,str);  // прочитали строку Рамановских активностей   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
raman[ch]:= copy(str,cn1+2+length('Raman Activ --'),cn2);   //первая интенсивность
raman[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Raman Activ --'),cn4);    //вторая интенсивность
raman[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Raman Activ --'),cn6);   //третья интенсивность
...
ReadLn(f,str);  // прочитали  деполяризацию Р   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
101:
...
depolar_p[ch]:= copy(str,cn1+2+length('Depolar (P) --'),cn2);   //первая интенсивность
depolar_p[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Depolar (P) --'),cn4);    //вторая интенсивность
depolar_p[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Depolar (P) --'),cn6);   //третья интенсивность
...
ReadLn(f,str);  // прочитали  деполяризацию U   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
102:
...
depolar_u[ch]:= copy(str,cn1+2+length('Depolar (U) --'),cn2);   //первая интенсивность
depolar_u[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Depolar (U) --'),cn4);    //вторая интенсивность
depolar_u[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Depolar (U) --'),cn6);   //третья интенсивность

end;



ReadLn(f,str);  // прочитали  Dip. str. (VCD)  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
103:
...
if copy(str,p,1)=' ' then cn1:=cn1+1         //пробелы до первой компонента
else break;
end;
for p:=length('Dip. str.   --')+2 to length(str) do  begin
if copy(str,cn1+p,1)<>' ' then cn2:=cn2+1       //первая компонента
else break;
end;
for p:=length('Dip. str.   --')+2 to length(str) do  begin
if copy(str,cn1+cn2+p,1)=' '  then cn3:=cn3+1      //пробелы до второй компонента
else break;
end;
for p:=length('Dip. str.   --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+p,1)<>' '  then cn4:=cn4+1   //вторая компонента
else break;
end;
for p:=length('Dip. str.   --')+2 to length(str) do  begin
if copy(str,cn1+cn2+cn3+cn4+p,1)=' ' then cn5:=cn5+1   //пробел до третьей компонента
else break;
end;
for p:=length('Dip. str.   --')+2 to length(str) do  begin
if (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>' ') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>'') and  (copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>#13) then cn6:=cn6+1  //третья компонента
end;

dip_str[ch]:= copy(str,cn1+2+length('Dip. str.   --'),cn2);   //первая компонента
dip_str[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Dip. str.   --'),cn4);    //вторая компонента
dip_str[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Dip. str.   --'),cn6);   //третья компонента

end;


ReadLn(f,str);  // прочитали  Rot. str. (VCD)  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
104:
...
rot_str[ch]:= copy(str,cn1+2+length('Rot. str.   --'),cn2);   //первая компонента
rot_str[ch+1]:= copy(str,cn1+cn2+cn3+2+length('Rot. str.   --'),cn4);    //вторая компонента
rot_str[ch+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Rot. str.   --'),cn6);   //третья компонента
...
/// Читаем смещшения координат ,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,
///////////////////////////////////////////////////////////////////////////////////
/// КООРДИНАТНЫЕ СМЕЩЕНИЯ ПО КОЛЕБАНИЯМ.............................................



ch_at:=0;

ReadLn(f,str);  // одну строку пропускаем
10:
ReadLn(f,str);
...
for p:=1 to length(str) do begin
if copy(str,p,1)=' ' then cn1:=cn1+1    //пробелы до порядкового номера
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+p,1)<>' ' then cn2:=cn2+1    //порядковый номер
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+p,1)=' ' then cn3:=cn3+1    //пробелы до атомного номера в таблице
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+p,1)<>' '  then cn4:=cn4+1    //атомный номер
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+p,1)=' '  then cn5:=cn5+1    //пробел до Х1
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+p,1)<>' ' then cn6:=cn6+1    //Х1
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+p,1)=' ' then cn7:=cn7+1    //пробелы до Y1
else break;
end;
for p:=1 to length(str) do begin
...
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+p,1)=' ' then cn9:=cn9+1    //пробелы до Z1
else break;
end;
for p:=1 to length(str) do begin
...
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+p,1)=' ' then cn11:=cn11+1    //пробелы до Х2
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+cn11+p,1)<>' ' then cn12:=cn12+1    //Х2
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+cn11+cn12+p,1)=' ' then cn13:=cn13+1    //пробелы до Y2
else break;
end;
for p:=1 to length(str) do begin
...
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+cn11+cn12+cn13+cn14+p,1)=' ' then cn15:=cn15+1    //пробелы до Z2
else break;
end;
for p:=1 to length(str) do begin
...
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+cn11+cn12+cn13+cn14+cn15+cn16+p,1)=' ' then cn17:=cn17+1    //пробелы до Х3
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+cn11+cn12+cn13+cn14+cn15+cn16+cn17+p,1)<>' ' then cn18:=cn18+1    //Х3
else break;
end;
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+cn11+cn12+cn13+cn14+cn15+cn16+cn17+cn18+p,1)=' ' then cn19:=cn19+1    //пробелы до Y3
else break;
end;
for p:=1 to length(str) do begin
...
for p:=1 to length(str) do begin
if copy(str,cn1+cn2+cn3+cn4+cn5+cn6+cn7+cn8+cn9+cn10+cn11+cn12+cn13+cn14+cn15+cn16+cn17+cn18+cn19+cn20+p,1)=' ' then cn21:=cn21+1    //пробелы до Z3
else break;
end;
for p:=1 to length(str) do begin
...
nom[i]:=copy(str,cn1+1,cn2);       //  порядковый номер
at_nom[i]:=copy(str,cn1+cn2+cn3+1,cn4);    // атомный номер
dx1[ch,i]:=copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);        //Х1
...
end;   // конец чтения координатных смещений

ch:=ch+3;
//basket[j]:=ch_at;


end;    // конец чтения секции частот

end;    // файл прочитан до конца
...
end;    // конец открытия диалога одного файла
...
if dial=false  then begin  // открыт диалог чтения нескольких файлов!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                           //  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
pol1_mult[j]:= copy(str,cn1+1,cn2);   //первое значение
pol2_mult[j]:= copy(str,cn1+cn2+cn3+1,cn4);    //второе значение
pol3_mult[j]:= copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);   //третье значение
...
if j>=1 then begin
maxj:=0;
for cic:=0 to j do       // cic - счетчик для перебора числа атомов во всех предыдущих системах для поиска наибольшего масива
if ch_mult[maxj]<ch_mult[cic] then maxj:=cic;   //maxj - индекс системы с наибольшим числом колебаний
...
frn_mult[j,ch_mult[j]]:= copy(str,cn1+1,cn2);   //первый номер
frn_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+1,cn4);    //второй номер
frn_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);   //третий номер
...
orb_mult[j,ch_mult[j]]:= copy(str,cn1+1,cn2);   //первый тип МО
orb_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+1,cn4);    //второй тип МО
orb_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);   //третий тип МО
...
if j>=1 then begin
//for cic:=0 to j do       // cic - счетчик для перебора числа атомов во всех предыдущих системах для поиска наибольшего масива
//if ch_mult[j]>ch_mult[cic] then maxj:=j;   //maxj - индекс системы с наибольшим числом колебаний
...
freq_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Frequencies --'),cn2);   //первая частота
freq_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Frequencies --'),cn4);    //вторая частота
freq_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Frequencies --'),cn6);   //третья частота
...
mass_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Red. masses --'),cn2);   //первая масса
mass_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Red. masses --'),cn4);    //вторая масса
mass_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Red. masses --'),cn6);   //третья масса
...
fr_const_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Frc consts  --'),cn2);   //первая константа
fr_const_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Frc consts  --'),cn4);    //вторая константа
fr_const_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Frc consts  --'),cn6);   //третья константа
...
ir_inten_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('IR Inten    --'),cn2);   //первая интенсивность
ir_inten_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('IR Inten    --'),cn4);    //вторая интенсивность
ir_inten_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('IR Inten    --'),cn6);   //третья интенсивность
...
ReadLn(f,str);  // прочитали строку Raman Intens   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
raman_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Raman Activ --'),cn2);   //первая интенсивность
raman_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Raman Activ --'),cn4);    //вторая интенсивность
raman_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Raman Activ --'),cn6);   //третья интенсивность
...
depolar_p_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Depolar (P) --'),cn2);   //первая деполяризация Р
depolar_p_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Depolar (P) --'),cn4);    //вторая деполяризация Р
depolar_p_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Depolar (P) --'),cn6);   //третья деполяризация Р
...
depolar_u_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Depolar (U) --'),cn2);   //первая интенсивность
depolar_u_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Depolar (U) --'),cn4);    //вторая интенсивность
depolar_u_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Depolar (U) --'),cn6);   //третья интенсивность
...
ReadLn(f,str);  // прочитали  Dip. str.   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
133:
cn1:=0;
cn2:=0;
cn3:=0;
...
dip_str_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Dip. str.   --'),cn2);   //первая интенсивность
dip_str_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Dip. str.   --'),cn4);    //вторая интенсивность
dip_str_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Dip. str.   --'),cn6);   //третья интенсивность
...
ReadLn(f,str);  // прочитали  Rot. str.   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
134:
cn1:=0;
cn2:=0;
cn3:=0;
...
rot_str_mult[j,ch_mult[j]]:= copy(str,cn1+2+length('Rot. str.   --'),cn2);   //первая интенсивность
rot_str_mult[j,ch_mult[j]+1]:= copy(str,cn1+cn2+cn3+2+length('Rot. str.   --'),cn4);    //вторая интенсивность
rot_str_mult[j,ch_mult[j]+2]:= copy(str,cn1+cn2+cn3+cn4+cn5+2+length('Rot. str.   --'),cn6);   //третья интенсивность
...
ch_at_mult[j]:=ch_at_mult[j]+1;        // подсчет числа атомов
...
maxj_at:=0;
for cic_at:=0 to j do       // cic - счетчик для перебора числа атомов во всех предыдущих системах для поиска наибольшего масива
if ch_at_mult[maxj_at]<ch_at_mult[cic_at] then maxj_at:=cic_at;   //maxj_at - индекс системы с наибольшим числом атомов


max_b:=0;
for cic_at:=0 to j do       // cic - счетчик для перебора числа атомов во всех предыдущих системах для поиска наибольшего масива
if max_b<basket[cic_at] then max_b:=basket[cic_at];   //maxj_at - индекс системы с наибольшим числом атомов
...
nom_mult[j,i]:=copy(str,cn1+1,cn2);       //  порядковый номер
at_nom_mult[j,i]:=copy(str,cn1+cn2+cn3+1,cn4);    // атомный номер
dx1_mult[j,ch_mult[j],i]:=copy(str,cn1+cn2+cn3+cn4+cn5+1,cn6);        //Х1
...
///////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////
//////////////////ЗДЕСЬ   НАЧИНАЕТ  ЗАПОЛНЯТЬСЯ ListView ДАНЫМИ
/////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////
...
if dial=true then begin        // если диалог открытия одного файла
...
Form24.ListView1.Columns.Add;
Form24.ListView1.Columns.Add;
Form24.ListView1.Columns.Items[4].Caption:='Рамановская интенсивность, А^4/а.е.';
Form24.ListView1.Columns.Items[5].Caption:='P-деполяризация';
Form24.ListView1.Columns.Items[6].Caption:='U-деполяризация';
...
else begin    // если диалог открытия нескольких файлов
...
for j:=0 to nf-1 do   begin
if ch_mult[j]=0 then MessageDlg('В файле '+ Form1.OpenDialog2.Files[j] +' отсутствуют данные о колебаниях',mtWarning,[mbOK],0);
if ch_mult[j]<>0 then Combobox1.Items.Add(Form1.OpenDialog2.Files[j]);
end;
Combobox1.ItemIndex:=0;

////////Теперь надо исключить нули и переиндексировать
...
if basket[i]=0 then FreeAndNil(basket[i]);  //убрали из массива лишние элементы    }
...
ListView1.Columns[4].Caption:='Рамановская интенсивность, А^4/а.е.';
ListView1.Columns[5].Caption:='P-деполяризация';
ListView1.Columns[6].Caption:='U-деполяризация';
BitBtn3.Enabled:=True;
BitBtn4.Enabled:=False;
...
///////////////// ЭТОТ БЛОК ДЛЯ ТОГО, ЧТОБЫ ИСКЛЮЧИТЬ ЭЛЕМЕНТЫ ТИПА ''
...
//  #     Type       Freq, cm^-1        Int, a.m.u.     Ram, А^4/a.m.u.      P-depolar         U-depolar      VCD, 10^-44 esu^2*cm^2
...
if cap='№' then cap:='#';
if cap='Тип' then cap:='     Type';
if cap='Частота, 1/см' then cap:='      Freq, cm^-1';
if cap='Интенсивность, о.е.' then cap:='         Int, a.m.u.';
if cap='Рамановская интенсивность, А^4/а.е.' then cap:='     Ram, А^4/a.m.u.';
if cap='P-деполяризация' then cap:='      P-depolar';
if cap='U-деполяризация' then cap:='         U-depolar';
...
//Проверяем установлен ли Excel на компьютере
begin
...
a,probel,cap: widestring;
begin
if dial=true then N4.Click   // если открыт изначально один файл с частотами
else begin
...
if ListView1.Columns[j].Caption='Рамановская интенсивность, А^4/а.е.' then Write(f,copy(probel,1,18-length(Raman_mult[basket[i],k])) + Raman_mult[basket[i],k]);
if ListView1.Columns[j].Caption='P-деполяризация' then Write(f,copy(probel,1,18-length(depolar_p_mult[basket[i],k])) + depolar_p_mult[basket[i],k]);
if ListView1.Columns[j].Caption='U-деполяризация' then Write(f,copy(probel,1,18-length(depolar_u_mult[basket[i],k])) + depolar_u_mult[basket[i],k]);
...
for j:=0 to Combobox1.Items.Count-1 do  begin     // цикл по файлам - закрывать в конце
...
if Raman_mult[basket[j],0]<>''  then begin
cell_i:=cell_i+1;
MSExcel.Cells[j+5+a,cell_i]:='Ram, А^4/a.m.u.';
end;
end;
end;
if depolar_p_mult<>nil then begin
