...
if (dial=true) and (Form1.OpenDialog1.FilterIndex=2) then begin       // если первый диалог и файлы типа out и log
j:=0;
...
sstr2:=copy(sstr1,0,i-1);               //номер орбитали
break;
end;
...
sstr4:=copy(sstr3,0,i-1);               //тип орбитали
break;
end;
...
sstr6:=copy(sstr5,0,i-1);               //орбитальная энергия
break;
end;
...
sstr8:=copy(sstr7,0,20);             //кинетическая энергия
...
showmessage('Файл '+ Form1.Opendialog1.FileName + ' не содержит данных об орбиталях');
goto 0;
end;
...
//////////////////ДОБАВЛЕНИЕ ИНФОРМАЦИИ В COMBOBOX////////////////////
...
end; ///////////////////////////////////////////// конец ситуации 1-го диалога
...
if dial=false then begin           // если выбран 2-ой диалог (мульти)
axx:=0;
...
sstr2:=copy(sstr1,0,s-1);               //номер орбитали
break;
end;
...
sstr4:=copy(sstr3,0,s-1);               //тип орбитали
break;
end;
...
sstr6:=copy(sstr5,0,s-1);               //орбитальная энергия
break;
end;
...
//basket:=nil;      //Корзина файлов с орбиталями среди всех возможно выбранных
//k:=0;
if  Ch_mult[i]>=1 then begin
setlength(basket,length(basket)+1);
basket[length(basket)-1]:=i;   // номер ненулевого элемента
Combobox1.Items.Add(task_mult[i]);
//k:=k+1;
end
else showmessage('Файл '+ Form1.OpenDialog2.Files[i] +' не содержит данных по орбиталям!');
//end;
...
///////////////////ЗАПОЛНЕНИЕ ТАБЛИЦЫ/////////////////////////////////
//////////////////при показе форму заполняется для первого прочитанного файла//////////////
///////////////////далее управление из формы при выборе файла из комбобокса//////////////

//if i=0 then begin   // условие того, что у нас файл с номером 1
...
Label10.Caption:=' = '+floattostrf(Simpleroundto(strtofloat(Eorb[homo])*ev-strtofloat(Eorb[homo-1])*ev,-4),ffFixed,5,3)+' эВ';
Label5.Caption:='Eвзмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb[homo-1])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo)+' / '+inttostr(ch-homo+2)+')';
Label6.Caption:='Eнсмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb[homo])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo+1)+' / '+inttostr(ch-homo+1)+')';
...
Label13.Caption:=#68;
Label19.Caption:=' = '+floattostrf(Simpleroundto(strtofloat(Eorb_beta[homo_beta])*ev-strtofloat(Eorb_beta[homo_beta-1])*ev,-4),ffFixed,5,3)+' эВ';
Label14.Caption:='Eвзмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_beta[homo_beta-1])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_beta)+' / '+inttostr(ch-homo_beta+2)+')';
Label15.Caption:='Eнсмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_beta[homo_beta])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_beta+1)+' / '+inttostr(ch-homo_beta+1)+')';
...
Label10.Caption:=' = '+floattostrf(Simpleroundto(strtofloat(Eorb_mult[basket[0],homo_mult[basket[0]]])*ev-strtofloat(Eorb_mult[basket[0],homo_mult[basket[0]]-1])*ev,-4),ffFixed,5,3)+' эВ';
Label5.Caption:='Eвзмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult[basket[0],homo_mult[basket[0]]-1])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult[basket[0]])+' / '+inttostr(ch_mult[basket[0]]-homo_mult[basket[0]]+2)+')';
Label6.Caption:='Eнсмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult[basket[0],homo_mult[basket[0]]])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult[basket[0]]+1)+' / '+inttostr(ch_mult[basket[0]]-homo_mult[basket[0]]+1)+')';
...
Label13.Font.Name:='Symbol';
Label13.Caption:=#68;
Label19.Caption:=' = '+floattostrf(Simpleroundto(strtofloat(Eorb_mult_beta[basket[0],homo_mult_beta[basket[0]]])*ev-strtofloat(Eorb_mult_beta[basket[0],homo_mult_beta[basket[0]]-1])*ev,-4),ffFixed,5,3)+' эВ';
Label14.Caption:='Eвзмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult_beta[basket[0],homo_mult_beta[basket[0]]-1])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult_beta[basket[0]])+' / '+inttostr(ch_mult[basket[0]]-homo_mult_beta[basket[0]]+2)+')';
Label15.Caption:='Eнсмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult_beta[basket[0],homo_mult_beta[basket[0]]])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult_beta[basket[0]]+1)+' / '+inttostr(ch_mult[basket[0]]-homo_mult_beta[basket[0]]+1)+')';
...
///////////////ЕСЛИ ДИАЛОГ ОТКРЫТИЯ ОДНОГО ФАЙЛА/////////////////////
probel:='                   ';
...
LineTo(43+15,0);
for i:=0 to scale do begin               //  scale=Round(PaintBox1.ClientHeight*0.1) - число насечек на оси энергий зависит от размеров окна
...
Label10.Caption:=' = '+floattostrf(Simpleroundto(strtofloat(Eorb_mult[basket[Combobox1.ItemIndex],homo_mult[basket[Combobox1.ItemIndex]]])*ev-strtofloat(Eorb_mult[basket[Combobox1.ItemIndex],homo_mult[basket[Combobox1.ItemIndex]]-1])*ev,-4),ffFixed,5,3)+' эВ';
Label5.Caption:='Eвзмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult[basket[Combobox1.ItemIndex],homo_mult[basket[Combobox1.ItemIndex]]-1])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult[basket[Combobox1.ItemIndex]])+' / '+inttostr(ch_mult[basket[Combobox1.ItemIndex]]-homo_mult[basket[Combobox1.ItemIndex]]+2)+')';
Label6.Caption:='Eнсмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult[basket[Combobox1.ItemIndex],homo_mult[basket[Combobox1.ItemIndex]]])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult[basket[Combobox1.ItemIndex]]+1)+' / '+inttostr(ch_mult[basket[Combobox1.ItemIndex]]-homo_mult[basket[Combobox1.ItemIndex]]+1)+')';
...
Label13.Font.Name:='Symbol';
Label13.Caption:=#68;
Label19.Caption:=' = '+floattostrf(Simpleroundto(strtofloat(Eorb_mult_beta[basket[Combobox1.ItemIndex],homo_mult_beta[basket[Combobox1.ItemIndex]]])*ev-strtofloat(Eorb_mult_beta[basket[Combobox1.ItemIndex],homo_mult_beta[basket[Combobox1.ItemIndex]]-1])*ev,-4),ffFixed,5,3)+' эВ';
Label14.Caption:='Eвзмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult_beta[basket[Combobox1.ItemIndex],homo_mult_beta[basket[Combobox1.ItemIndex]]-1])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult_beta[basket[Combobox1.ItemIndex]])+' / '+inttostr(ch_mult[basket[Combobox1.ItemIndex]]-homo_mult_beta[basket[Combobox1.ItemIndex]]+2)+')';
Label15.Caption:='Eнсмо = '+Floattostrf(Simpleroundto(strtofloat(Eorb_mult_beta[basket[Combobox1.ItemIndex],homo_mult_beta[basket[Combobox1.ItemIndex]]])*ev,-4),ffFixed,5,3) +' эВ'+'       '+ '('+inttostr(homo_mult_beta[basket[Combobox1.ItemIndex]]+1)+' / '+inttostr(ch_mult[basket[Combobox1.ItemIndex]]-homo_mult_beta[basket[Combobox1.ItemIndex]]+1)+')';
